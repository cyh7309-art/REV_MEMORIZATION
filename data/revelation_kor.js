/* 요한계시록 개역한글 본문 데이터 (자동 생성)
   - data/revelation_kor.json 과 내용이 동일합니다.
   - file:// 로 직접 열 때는 fetch()가 막히므로 이 파일이 사용됩니다.
   - 본문을 교체하려면 두 파일을 함께 바꾸거나, 앱의 [설정 > 본문 데이터 불러오기]에서 JSON을 올리세요. */
window.REVELATION_DATA = {
 "book": "요한계시록",
 "version": "개역한글",
 "source": "대한성서공회 『성경전서 개역한글판』(1961). 저작재산권 보호기간(50년) 만료로 저작권료 없이 사용 가능 — 대한성서공회 저작권 FAQ. 본문 수집: YouVersion bible.com KRV(id 88), 2026-08-21. 교차검증: MaatheusGois/bible ko 데이터셋(MIT).",
 "verseCounts": {
  "1": 20,
  "2": 29,
  "3": 22,
